defmodule OOTPUtilityWeb.Helpers.PathTest do
  ...
  describe "path_to_conference" do
    test "it returns the path to the conference, nested in its parent league, both identified by their slugs",
         %{conn: conn} do
      league = build(:league, slug: "my-league")
      conference = build(:conference, league: league, slug: "my-conference")

      assert Path.path_to_conference(conference, conn) ==
               "/leagues/my-league/conferences/my-conference"
    end
  end

  describe "path_to_division" do
    test "it returns the path to the division, nested in its parent league, both identified by their slugs, when the division has no conference",
         %{conn: conn} do
      league = build(:league, slug: "my-league")
    ...
    test "it returns the path to the division, nested in its parent league and conference, all identified by their slugs, when the division has a conference",
         %{conn: conn} do
      league = build(:league, slug: "my-league")
      conference = build(:conference, league: league, slug: "my-conference")
      division = build(:division, slug: "my-division", league: league, conference: conference)

      assert Path.path_to_division(division, conn) ==
               "/leagues/my-league/conferences/my-conference/divisions/my-division"
    end
  end
end
