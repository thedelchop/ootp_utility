defmodule OOTPUtility.Game.LogTest do
    ...
    test "returns all of the raw_text from all lines which are not formatted" do
      foul_ball =
        Fixtures.create_game_log_line(%{
          raw_text: "0-0: Foul Ball, location: 2F",
          formatted_text: nil
        })

      strike_out =
        Fixtures.create_game_log_line(%{raw_text: "1-2: Strikes Out Looking", formatted_text: nil})
  ...
  describe "format_lines" do
    test "it formats all Lines for whom the \"formatted_text\" can be produced" do
      ...
      unformattable_line =
        Fixtures.create_game_log_line(%{
          raw_text: "1-2: Caught stealing home!",
          formatted_text: nil
        })
    ...
    test "it only formats Lines who are included in the query if a query was included" do
      ...
      formattable_but_skipped_line =
        Fixtures.create_game_log_line(%{
          raw_text: "0-1: Foul Ball, location: 2F",
          formatted_text: nil
        })
      ...
      Log.format_lines(
        from l in Line, where: l.id in [^formattable_line.id, ^unformattable_line.id]
      )
