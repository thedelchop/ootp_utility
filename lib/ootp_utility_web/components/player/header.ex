defmodule OOTPUtilityWeb.Components.Player.Header do
  ...
  def render(assigns) do
    ...
    <Section event_target={@socket.id}>
      <div class="flex gap-1 md:gap-2">
        <img
          class="h-16 w-16 md:h-28 md:w-28 rounded-full md:mt-2"
          src={Routes.static_path(@socket, "/images/default_player_photo.jpg")}
          alt=""
        />
            ...
            <div class="flex flex-row items-center gap-1 md:gap-2 text-base md:text-lg font-medium text-gray-900">
              <LiveRedirect to={path_to_team(@team, @socket)} class="contents">
                <div class="flex-shrink-0 h-6 md:h-8 h-6 md:w-8">
                  <img
                    class="h-6 md:h-8 w-6 md:w-8 rounded-full"
  ...
  def update(%{player: player} = _assigns, socket) do
    {
      :ok,
      socket
